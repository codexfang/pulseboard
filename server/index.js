...
async function measureLatency(url, attempt = 0) {
  ...
  try {
    ...
    return { success: response.ok, statusCode: response.status, latency, payloadSize: body.length, timestamp: Date.now(), error: null };
  } catch (err) {
    clearTimeout(timeout);
    if (err.name === 'AbortError') {
      return { success: false, statusCode: 0, latency: 15000, payloadSize: 0, timestamp: Date.now(), error: 'Request timed out' };
    }
    if (attempt < RETRY_ATTEMPTS) {
      await new Promise((r) => setTimeout(r, RETRY_DELAY * (attempt + 1)));
      return measureLatency(url, attempt + 1);
    }
    return { success: false, statusCode: 0, latency: Math.round(performance.now() - start), payloadSize: 0, timestamp: Date.now(), error: err.message };
  }
}

async function pollAll() {
  const promises = SERVICES.map(async (svc) => {
    const url = `${svc.endpoint}${svc.checkPath}`;
    const result = await measureLatency(url);
    results.set(svc.id, result);
    let h = history.get(svc.id) || [];
    h.push({ ...result, serviceId: svc.id });
    if (h.length > 500) h = h.slice(-500);
    history.set(svc.id, h);
  });
  await Promise.allSettled(promises);
  console.log(`[${new Date().toISOString()}] Polled ${SERVICES.length} services`);
}

function computeHealthScore(latency, errorRate, uptime) {
  const ls = latency <= 800 ? 100 : Math.max(0, 100 - ((latency - 800) / 2000) * 50);
  const es = errorRate <= 0.02 ? 100 : Math.max(0, 100 - (errorRate / 0.15) * 60);
  const us = uptime * 100;
  return Math.round(ls * 0.35 + es * 0.35 + us * 0.3);
}

function getEnrichedState() {
  return SERVICES.map((svc) => {
    const last = results.get(svc.id);
    const h = history.get(svc.id) || [];
    const latencies = h.map((r) => r.latency);
    const avgLatency = latencies.length > 0 ? Math.round(latencies.reduce((a, b) => a + b, 0) / latencies.length) : 0;
    const errRate = h.length > 0 ? h.filter((r) => !r.success).length / h.length : 0;
    const uptime = h.length > 0 ? h.filter((r) => r.success).length / h.length : 1;
    const score = computeHealthScore(avgLatency, errRate, uptime);
    let status = 'healthy';
    if (score < 80) status = score >= 50 ? 'degraded' : 'failing';
    return { ...svc, last, history: h, avgLatency, errorRate: errRate, uptime, healthScore: score, status, totalRequests: h.length };
  });
}
...
app.get('/api/status', (_req, res) => {
  res.json({ services: getEnrichedState(), incidentMode: false, lastPoll: Date.now() });
});
...
app.get('/api/history/:serviceId', (req, res) => {
  const h = history.get(req.params.serviceId) || [];
  res.json(h.slice(-100));
});
...
app.get('/api/export', (_req, res) => {
  res.json({
    ...
    version: '1.0.0',
    services: getEnrichedState().map((s) => ({
      id: s.id, name: s.name, health: { score: s.healthScore, status: s.status, latency: s.last?.latency },
      history: (history.get(s.id) || []).map((r) => ({ ts: r.timestamp, latency: r.latency, success: r.success, statusCode: r.statusCode })),
    })),
  });
});
